...
export default function CheckoutPage() {
  ...
  const handleEircodeSearch = async () => {
    ...
    try {
      ...
      if (parsed.postcode) { setPostcode(parsed.postcode); setEircodeInput(parsed.postcode); }
      else setPostcode(eircodeInput.toUpperCase());
  ...
  const inp = (hasError) => ({
    ...
    fontSize: '16px', outline: 'none', fontFamily: 'var(--font-body)', width: '100%',
    boxSizing: 'border-box', WebkitTextFillColor: 'var(--text)', transition: 'border-color 0.18s',
  });

  const card = { background: 'var(--surface)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border-light)', padding: '20px', boxShadow: 'var(--shadow-sm)' };
          ...
          {['Contact', 'Delivery', 'Review'].map((label, i) => (
            <React.Fragment key={label}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <div style={{ width: 28, height: 28, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, background: step >= i + 1 ? 'var(--primary)' : 'var(--surface-2)', color: step >= i + 1 ? '#fff' : 'var(--text-muted)' }}>
                  {step > i + 1 ? '✓' : i + 1}
                </div>
            ...
            <div style={card}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                <div className="form-group">
                  ...
                  <input type="text" placeholder="Your full name" value={name} onChange={e => setName(e.target.value)} style={inp(errors.name)} onFocus={e => e.target.style.borderColor = 'var(--primary)'} onBlur={e => e.target.style.borderColor = 'var(--border)'} />
                  {errors.name && <span style={{ fontSize: 12, color: 'var(--danger)' }}>Required</span>}
                </div>
                <div className="form-group">
                  <label className="form-label">Phone Number *</label>
                  <input type="tel" placeholder="+353 87 123 4567" value={phone} onChange={e => setPhone(e.target.value)} style={inp(errors.phone)} onFocus={e => e.target.style.borderColor = 'var(--primary)'} onBlur={e => e.target.style.borderColor = 'var(--border)'} />
                  {errors.phone && <span style={{ fontSize: 12, color: 'var(--danger)' }}>Required</span>}
                </div>
              </div>
            </div>
            <button className="btn btn-primary btn-lg" style={{ width: '100%' }} onClick={handleNext}>Continue to Delivery →</button>
            {!user && <p style={{ textAlign: 'center', fontSize: 13, color: 'var(--text-muted)' }}>Have an account? <span style={{ color: 'var(--primary)', cursor: 'pointer', fontWeight: 600 }} onClick={() => navigate('/login')}>Sign in</span></p>}
          </div>
        )}
        ...
        {step === 2 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ ...card, background: 'linear-gradient(135deg, #f0f7f3, #fdfaf5)', border: '1.5px solid #c8e6d4' }}>
              <p style={{ fontSize: 14, fontWeight: 700, color: 'var(--primary)', marginBottom: 6 }}>🔍 Eircode Lookup</p>
              <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 12 }}>Enter your Eircode to automatically fill your address</p>
              <div style={{ display: 'flex', gap: 8 }}>
                <input type="text" placeholder="e.g. T12 A123" value={eircodeInput} onChange={e => setEircodeInput(e.target.value.toUpperCase())} onKeyDown={e => e.key === 'Enter' && handleEircodeSearch()} style={{ ...inp(false), flex: 1, letterSpacing: '0.06em' }} maxLength={8} />
                <button className="btn btn-primary" onClick={handleEircodeSearch} disabled={eircodeLoading} style={{ flexShrink: 0, minWidth: 80 }}>{eircodeLoading ? '…' : 'Find'}</button>
              </div>
              {eircodeFound && <p style={{ fontSize: 12, color: 'var(--primary)', marginTop: 8, fontWeight: 600 }}>✓ Address filled below — check and correct if needed</p>}
            </div>
            <div style={card}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                <div className="form-group">
                  <label className="form-label">Address Line 1 *</label>
                  <input type="text" placeholder="14 Oak Drive" value={line1} onChange={e => setLine1(e.target.value)} style={inp(errors.line1)} onFocus={e => e.target.style.borderColor = 'var(--primary)'} onBlur={e => e.target.style.borderColor = 'var(--border)'} />
                  {errors.line1 && <span style={{ fontSize: 12, color: 'var(--danger)' }}>Required</span>}
                </div>
                <div className="form-group">
                  <label className="form-label">Address Line 2 (optional)</label>
                  <input type="text" placeholder="Area or townland" value={line2} onChange={e => setLine2(e.target.value)} style={inp(false)} />
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  <div className="form-group">
                    <label className="form-label">City / Town *</label>
                    <input type="text" placeholder="Cork" value={city} onChange={e => setCity(e.target.value)} style={inp(errors.city)} onFocus={e => e.target.style.borderColor = 'var(--primary)'} onBlur={e => e.target.style.borderColor = 'var(--border)'} />
                    {errors.city && <span style={{ fontSize: 12, color: 'var(--danger)' }}>Required</span>}
                  </div>
                  <div className="form-group">
                    <label className="form-label">County</label>
                    <input type="text" placeholder="Cork" value={county} onChange={e => setCounty(e.target.value)} style={inp(false)} />
                  </div>
                </div>
                <div className="form-group">
                  <label className="form-label">Eircode *</label>
                  <input type="text" placeholder="T12 A123" value={postcode} onChange={e => setPostcode(e.target.value.toUpperCase())} style={{ ...inp(errors.postcode), letterSpacing: '0.06em' }} maxLength={8} />
                  {errors.postcode && <span style={{ fontSize: 12, color: 'var(--danger)' }}>Required</span>}
                </div>
                <div className="form-group">
                  <label className="form-label">Delivery Instructions</label>
                  <input type="text" placeholder="Ring bell, leave at door…" value={instructions} onChange={e => setInstructions(e.target.value)} style={inp(false)} />
                </div>
                <div className="form-group">
                  <label className="form-label">Order Notes</label>
                  <textarea placeholder="Any special requests…" value={notes} onChange={e => setNotes(e.target.value)} style={{ ...inp(false), minHeight: 72, resize: 'vertical' }} />
                </div>
              </div>
            </div>
            <button className="btn btn-primary btn-lg" style={{ width: '100%' }} onClick={handleNext}>Review Order →</button>
          </div>
        )}
        ...
        {step === 3 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={card}>
              ...
              {items.map(i => (
                <div key={i.product_id} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                  <div><p style={{ fontSize: 14, fontWeight: 600 }}>{i.product_name}</p><p style={{ fontSize: 12, color: 'var(--text-muted)' }}>{i.unit_label} × {i.quantity}</p></div>
                  <p style={{ fontWeight: 700, color: 'var(--primary)' }}>€{i.line_total.toFixed(2)}</p>
                </div>
              ))}
              <hr />
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, marginTop: 10 }}><span style={{ color: 'var(--text-muted)' }}>Subtotal</span><span>€{subtotal.toFixed(2)}</span></div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, marginTop: 8 }}><span style={{ color: 'var(--text-muted)' }}>Delivery</span><span>{delivery === 0 ? <span style={{ color: 'var(--primary)', fontWeight: 700 }}>FREE</span> : `€${delivery.toFixed(2)}`}</span></div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 800, fontSize: 20, marginTop: 14 }}><span>Total</span><span style={{ color: 'var(--primary)', fontFamily: 'var(--font-display)' }}>€{total.toFixed(2)}</span></div>
            </div>
            <div style={card}>
