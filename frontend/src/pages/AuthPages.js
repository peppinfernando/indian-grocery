...
export function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showToast('Invalid credentials — check your phone number or name');
  ...
  const inp = {
    ...
    WebkitTextFillColor: 'var(--text)', boxSizing: 'border-box'
  };
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div className="form-group">
              ...
              <input style={inp} type="text"
                placeholder="+353 87 123 4567 or your name"
                value={identifier}
                onChange={e => setIdentifier(e.target.value)}
                onFocus={e => e.target.style.borderColor = 'var(--primary)'}
                onBlur={e => e.target.style.borderColor = 'var(--border)'}
                required />
              <p style={{ fontSize: 11, color: 'var(--text-light)', marginTop: 3 }}>You can sign in with your registered phone number or full name</p>
            </div>
            <div className="form-group">
...
function PhoneInput({ value, onChange }) {
  return (
    <div style={{ display: 'flex', borderRadius: 'var(--radius-md)', overflow: 'hidden', border: '1.5px solid var(--border)' }}>
      <div style={{
        background: 'var(--surface-2)', padding: '13px 12px',
        fontSize: 13, color: 'var(--text-muted)', fontWeight: 600,
        whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: 5,
        flexShrink: 0, userSelect: 'none', borderRight: '1.5px solid var(--border)',
      }}>
        🇮🇪 +353
      </div>
      <input
        style={{
          padding: '13px 12px', border: 'none', background: 'var(--surface)',
          color: 'var(--text)', fontSize: 15, outline: 'none',
          fontFamily: 'var(--font-body)', flex: 1, width: '100%',
          WebkitTextFillColor: 'var(--text)', boxSizing: 'border-box',
        }}
        type="tel"
        ...
        <div className="card" style={{ padding: '28px 24px' }}>
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div className="form-group">
              ...
              <p style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>Irish number — enter digits after +353</p>
            </div>
            <div className="form-group">
              <label className="form-label">Password</label>
