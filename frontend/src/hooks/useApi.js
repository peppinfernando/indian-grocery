...
// Works for both localhost and phone on same WiFi
// When accessed via network IP, calls backend on port 8000 of same host
const getBaseURL = () => {
  const hostname = window.location.hostname;
  if (hostname === 'localhost' || hostname === '127.0.0.1') {
    return '/api'; // use proxy
  }
  return `http://${hostname}:8000/api`; // direct to backend for phone/network access
};
