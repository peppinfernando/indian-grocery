    ...
    <header className="top-header">
      <div className="header-inner">
        ...
        <form onSubmit={handleSearch} style={{ flex: 1, maxWidth: 380, margin: '0 20px' }}>
          <div style={{ position: 'relative' }}>
            ...
            <input
              ...
              style={{ width: '100%', padding: '9px 14px 9px 34px', fontSize: 13, background: 'var(--surface-2)' }}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          {user ? (
            <>
              {user.is_admin && <button className="btn btn-outline btn-sm" onClick={() => navigate('/admin')}>Admin</button>}
              <button className="btn btn-ghost btn-sm" onClick={() => navigate('/account')}>👤 {user.name.split(' ')[0]}</button>
              <button className="btn btn-ghost btn-sm" onClick={logout}>Sign out</button>
            </>
          ) : (
            <button className="btn btn-ghost btn-sm" onClick={() => navigate('/login')}>Sign In</button>
          )}
          <button className="btn btn-primary btn-sm" onClick={() => navigate('/cart')} style={{ gap: 6 }}>
            🛒 {totalItems > 0 && <span style={{ background: 'rgba(255,255,255,0.25)', borderRadius: 6, padding: '1px 6px', fontSize: 11, fontWeight: 700 }}>{totalItems}</span>}
          </button>
        </div>
      </div>
    </header>
    ...
    >
      ...
      <div style={{ padding: '12px 14px 14px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <span style={{ fontSize: 16, fontWeight: 700, color: 'var(--primary)', fontFamily: 'var(--font-display)' }}>€{price.toFixed(2)}</span>
            {hasDiscount && <span style={{ fontSize: 11, color: 'var(--text-light)', textDecoration: 'line-through', marginLeft: 5 }}>€{product.price.toFixed(2)}</span>}
            <p style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 1 }}>{product.unit_label}</p>
          </div>
          <button
            className="btn btn-primary"
            style={{ borderRadius: '50%', width: 32, height: 32, padding: 0, fontSize: 18, flexShrink: 0 }}
            onClick={handleAdd}
